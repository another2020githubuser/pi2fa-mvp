'''Sip Uri Parser, including validation'''
...
class SipUriParser:
    ...
    def parseSipUri(self, sip_uri_str):
        '''Parses SIP uri.  No validation done'''
...
class SipUri:
    '''data transer object represents a parsed sip uri'''
    ...
    #Warning:  __cmp__ has been removed from Python 3, __eq__ is used instead
    #this affects the IN() operator
...
class UriValidator:
    ...
    #these were in endpoint.py, but they seemed better here
    def validateUri(self, uri):
        '''Validates a uri.
        Note:  Endpoint::libCreate() must have been called or this function will crash.'''
        self._logger.debug("enter validateUri")
        pjsip_accessor = PjSipContainerAccessor()
        instance = pjsip_accessor.endpoint
